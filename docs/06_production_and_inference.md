...
The optimization strategies in this module revolve entirely around avoiding or minimizing this VRAM transfer bottleneck.
...
By compressing the weights to 8-bit, we halve the size of the model in VRAM and double the speed at which weights can be loaded across the memory bus. However, this is a "lossy compression". The mathematical challenge, addressed by algorithms like **GPTQ** or **AWQ**, is finding ways to represent complex numbers with very few integer buckets without the neural network losing its accuracy.
...
This perfectly identical generation guarantees output quality while achieving 2x-3x speedups in real-world scenarios.
...
This decoupling enables **Continuous Batching**, where the server dynamically injects new requests the exact microsecond an old request finishes, keeping GPU utilization incredibly high and increasing API serving throughput by orders of magnitude.
