...
> [!IMPORTANT]
> **Why we build the full Encoder–Decoder first.** This chapter and notebooks 11–13 build the *historical* 2017 Encoder–Decoder Transformer, because seeing both halves is the clearest way to learn each component. From the full Transformer (notebook 12) onward we keep **only the Decoder stack** (the Llama-style, decoder-only design used by every modern chat model). **Cross-Attention in particular will be deleted** — learn it for understanding the original architecture, not because the final model uses it.
